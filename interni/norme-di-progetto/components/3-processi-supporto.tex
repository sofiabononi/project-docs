...
Al fine di organizzare coerentemente e uniformemente la struttura dei documenti, si è deciso di utilizzare il \textit{package} \textbf{subfiles} di \LaTeX. La \glossario{repository} \texttt{project-docs} contiene una \textit{directory} denominata \texttt{commons/} con al suo interno:
\begin{itemize}
...
In accompagnamento ai documenti ufficiali viene fornito un \textsc{Glossario v1.4-1.2.0}, contenente tutti i termini caratterizzati da sottolineatura e una G maiuscola come pedice. Il glossario ha la funzione di chiarire possibili fraintendimenti provenienti dall'utilizzo di determinati termini. \\
...
\begin{itemize}
  ...
  \item Utilizzo di testo sottolineato, in aggiunta al pedice G maiuscola, per i termini appartenenti al \textsc{Glossario v1.4-1.2.0};
...
\begin{itemize}
  ...
  \item \texttt{analisi-dei-requisiti}: branch secondario nel quale viene sviluppato l'intero documento \textsc{Analisi dei Requisiti v1.4-1.2.1};
  \item \texttt{glossario}: branch secondario nel quale viene sviluppato l'intero documento \textsc{Glossario v1.4-1.2.0};
  ...
  \item \texttt{piano-di-qualifica}: branch secondario nel quale viene sviluppato l'intero documento \textsc{Piano di Qualifica v1.4-2.0.0};
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item La cifra massima corrisponde al massimo numero di requisiti riportati nell'\textsc{Analisi dei Requisiti v1.4-1.2.1}.
    \end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{\glossario{Travis CI}} per le operazioni di controllo basate sui test definiti nel \textsc{Piano di Qualifica v1.4-2.0.0} e il supporto al versionamento dei documenti e delle componenti software.
\end{itemize}
...
\end{itemize}
\subsubsection{Descrizione}
La gestione della qualità viene trattata nello specifico nel \textsc{Piano di Qualifica v1.4-2.0.0}. In particolare, in tale documento:
\begin{itemize}
...
\end{itemize}
Il controllo con questo strumento viene eseguito ad ogni pull request di un branch su un branch principale (\texttt{master} o \texttt{develop}).
...
I test di controllo vengono eseguiti da questo strumento ad ogni push su un qualsiasi branch della repository e ad ogni pull request di un branch su un branch principale (\texttt{master} o \texttt{develop}).
...
\begin{itemize}
  \item \textbf{Descrizione}: questa metrica identifica la percentuale di test implementati e soddisfatti tra quelli definiti dal gruppo nel \textsc{Piano di Qualifica v1.4-2.0.0}.
