...
Google Sheets è un programma, appartenente alla \textit{suite office} di Google, per la creazione di fogli di calcolo. Questo applicativo viene usato dal gruppo per la creazione delle tabelle e dei grafici presenti nel \textsc{Piano di Progetto}, e per lo svolgimento dei calcoli dei preventivi presenti nello stesso documento.
...
I requisiti vengono classificati e identificati secondo uno schema univoco, al fine di facilitarne la comprensione. La classificazione segue la seguente codifica:
\begin{center}
  \centering
  \textbf{R[P]-[T]-[I]}
\end{center} dove:
\begin{itemize}
  ...
  \begin{itemize}
    \item \textbf{A}: indica un requisito obbligatorio poiché necessario;
    \item \textbf{B}: indica un requisito desiderabile, ma non necessario;
    \item \textbf{C}: indica un requisito opzionale o contrattabile in corso d'opera del progetto.
  \end{itemize}
  \item \textbf{T}: indica la tipologia del requisito; questa può essere:
  \begin{itemize}
...
Anche i casi d'uso sono identificati secondo uno schema univoco; questo è così composto:
\begin{center}
  \centering
  \textbf{UC[C].[CsC]}
\end{center} dove:
\begin{itemize}
  ...
  \item \textbf{CsC}: consistente in un numero progressivo, identifica il sottocaso.
\end{itemize}
Ogni caso d'uso deve contenere i seguenti campi:
\begin{itemize}
  \item %%% IN ATTESA DELL'AdR %%%
\end{itemize}
...
%%% IN ATTESA DELL'AdR O DI PARLARNE %%%
\paragraph{Codifica}
\subparagraph{Scopo}
