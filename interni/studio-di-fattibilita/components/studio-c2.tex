...
agli utenti finali di pagare per la loro esecuzione. Si basa sul concetto di \glossario{Computation-as-a-Service} (o \textit{CaaS}).
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Configurare conto \glossario{Ethereum};
  ...
  \end{itemize}
  \item \textbf{etherless-smart}: è un set di \glossario{Smart Contracts} che permettono di gestire la comunicazione tra
  ...
  \item \textbf{etherless-server}: il modulo che esegue le funzioni e attraverso \textit{etherless-smart} comunica
        ...
        emette un evento nella \glossario{Blockchain}, tale evento verrà ricevuto da \textit{etherless-cli} che mostrerà il risultato
        allo sviluppatore.
\end{itemize}

Inoltre il progetto richiede i seguenti vincoli:
\begin{itemize}
      \item \textit{etherless-cli} deve essere installato utilizzando il \glossario{node package manager} (\textit{npm}) utilizzando
      ...
      \item Devono essere utilizzati \glossario{typescript-eslint} ed \glossario{ESLint} durante il processo;
      \item \textit{etherless-server} deve essere implementato utilizzando un \glossario{framework serverless}.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Ethereum}: è una piattaforma che permette agli utenti di scrivere agevolmente \glossario{applicazioni decentralizzate} (ĐApps) utilizzando la \textit{Blockchain};
  \item \textbf{Blockchain}: è un database condiviso che tiene traccia di tutti i movimenti di \glossario{criptovaluta} utilizzando una struttura a blocchi. Una volta che un blocco è stato verificato esso viene inserito irreversibilmente nella \textit{Blockchain};
