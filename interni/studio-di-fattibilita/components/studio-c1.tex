...
Il capitolato C1 prevede la creazione di una \glossario{piattaforma web} che mediante l'inserimento di un video relativo ad un evento sportivo identificato, riesca a produrre un ulteriore video della durata massima di 5 minuti, contenente i momenti salienti (\glossario{highlights}) del video precedentemente inserito.

\subsection{Finalità del progetto}%
\label{sub:finalita_del_progetto}
Il \glossario{progetto} consiste nel creare una piattaforma web dotata di un sistema di Machine Learning in grado di identificare ogni momento importante di un evento.
...
\begin{itemize}
  ...
  \item Realizzare l'\glossario{architettura} basata a \glossario{micro-servizi}, in modo da dividere il progetto in diverse funzionalità ognuna indipendente dalle altre;
...
\begin{itemize}
  ...
  \item \glossario{DynamoDB}: database \glossario{NoSQL} dalle alte performance ideale per la conservazione di tag o altre informazioni a supporto dell'applicativo;
  ...
  \end{itemize}
  \item \glossario{AWS Rekognition video}: servizio di analisi video basato su apprendimento approfondito, in grado di:
  \begin{itemize}
...
\begin{itemize}
    \item \glossario{Node.js}: per lo sviluppo di \glossario{\textit{API Restful}} JSON a supporto dell’applicativo;
...
\begin{itemize}
  ...
  \item \glossario{CSS3};
  \item \glossario{Javascript}, utilizzando un \glossario{framework} \glossario{\textit{responsive}} come \glossario{Twitter}.
\end{itemize}
